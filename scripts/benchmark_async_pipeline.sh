...
docker run --rm --network=none --cpuset-cpus="${CPUSET_CPUS:-0-7}" \
  ...
  -e MODES="${MODES:-sync_both,async_handoff,async_protobuf,async_pipe,async_both}" \
  -e RESULTS_DIR=/work/results \
  -v "${RESULTS_DIR}:/work/results" \
  --entrypoint /work/scripts/container_async_pipeline_benchmark.sh "${IMAGE_NAME}"
echo "Done: ${RESULTS_DIR}/ASYNC_PIPELINE_REPORT.md"
